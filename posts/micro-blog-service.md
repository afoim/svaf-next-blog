...
description: 很久很久以前，我的网站一直都是把博文和图片资源当做源码，封装在整个源码仓库里的。这会导致仅仅更新一行代码，CI/CD 都需要重新拉取整个仓库，包括巨占空间的博客图片。那么，我们为什么不能跟它分离开来？来，这期教你掌控分离的权能！
...
那么最终我选择第三种方案，就是通过和论坛一样的查询 query 来实现。由于 Next.js 要求每一个路径、每一个 path name 都需要对应一个唯一的 HTML，那么我们就把 posts.html 写好，然后通过 ?slug=某一篇文章，这样来将所有文章都定向到 posts。通过动态的查询参数来获取不同的文章。但这又导致一个问题，但这个问题我们是可以解决的，那就是我们的评论区，也就是 giscus 依赖于 pathname。这样改完以后，所有的文章的 pathname 都是 posts，因为 pathname 严格意义上并不会把 query 包含在内。
